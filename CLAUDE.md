...
- **Phase 1D — done (2026-05-05).** AWS infrastructure live; site serving https://scottclark.io. Terraform module at `terraform/` provisions everything between the existing Route 53 hosted zone and GitHub Actions: ACM cert (apex + `*.scottclark.io`, DNS-validated automatically during apply), private S3 bucket (versioning + 90-day noncurrent expiration + OAC-only access via CloudFront source-ARN policy), CloudFront distribution (PriceClass_100, http2and3) with an edge function for www→apex 301 + /about→/ 301 + clean-URL → /index.html rewrite, response-headers policy (HSTS 1y, X-Frame-Options DENY, Referrer-Policy strict-origin-when-cross-origin, Permissions-Policy FLoC/Topics opt-out), Route 53 alias records (apex + www, A + AAAA), GitHub OIDC provider + deploy IAM role scoped to `repo:sc932/scottclarkio:ref:refs/heads/main` only. The hosted zone is consumed as a `data` source — Terraform doesn't recreate it, preserving the migrated MX (Google Workspace) and TXT records. GH Actions workflow `.github/workflows/deploy.yml` runs on every push to main: npm ci → astro build → two-pass S3 sync (hashed assets `max-age=31536000, immutable`; HTML/XML/JSON `max-age=3600, must-revalidate`) → CloudFront `/*` invalidate. Three repo secrets feed it (`AWS_ROLE_ARN`, `S3_BUCKET`, `CLOUDFRONT_DISTRIBUTION_ID`) — set from `terraform output` via `gh secret set`. DNS migrated iwantmyname → Route 53 on 2026-05-04, propagated 2026-05-05 (one false start where iwantmyname's UI silently dropped the save on first try; second attempt took globally within minutes). **AWS credential bridge for Terraform**: Scott authenticates AWS CLI via `aws login` (modern `login_session` config in `~/.aws/config`), but Terraform's AWS provider doesn't natively read that. Bridge with `eval "$(aws configure export-credentials --format env)"` before any `terraform plan`/`apply`/`init -migrate-state` — exports temporary STS creds into env vars the provider does recognize. Session expires on `aws login`'s TTL; if Terraform stalls mid-apply, re-export and re-run (apply is idempotent).
- **Phase 2 — done (2026-07-28).** Blog LIVE at `/blog` on the estate grammar (vault `Projects/content_factory/blog-grammar.md` is the canonical contract): pillar-enum collection (`startup-lessons | building-things` display taxonomy), breadcrumbs + `?pillar=` sort view, TitleParts, per-post `.md` twins + OG cards + full-content RSS + llms surfaces, aio-check + check-crosslinks gates in CI. Launch pair (2026-07-28): make-all-new-mistakes-faster + building-this-blog-part-ii; content ports from vault `Writing/<slug>.md` at Scott's LOCK (the MDX header comments carry the ported version). Nav label "Writing" (provisional pick).
...
- **AWS S3 + CloudFront** — static hosting and CDN. Provisioned by Terraform (`terraform/`), live as of 2026-05-05.
- **GitHub Actions** — `.github/workflows/deploy.yml` deploys on every push to main via OIDC role assumption (no long-lived AWS keys in the repo). Action versions pinned exact to the Node-24-compatible series ahead of GitHub's June 2026 Node 20 deprecation: `actions/checkout@v6.0.0`, `actions/setup-node@v6.0.0`, `aws-actions/configure-aws-credentials@v6.0.0`. Per dep-hygiene rules, action bumps follow the same "oldest version that fixes the issue, ≥2 weeks aged" pattern as npm.
- **Terraform** — infrastructure-as-code for the AWS deploy; `terraform >= 1.7`, `hashicorp/aws ~> 5.80`. State stored in private S3 bucket `scottclarkio-terraform-state` (Phase 1D postscript). `.terraform.lock.hcl` is committed (provider lockfile, treated like `package-lock.json`).
...
scripts/deploy.sh       Manual deploy alternative — sources S3_BUCKET +
                        CLOUDFRONT_DISTRIBUTION_ID from .env, uses local
                        AWS CLI auth. Normally unused since GH Actions
                        handles deploys; kept for emergency hand-deploy.
terraform/              Infrastructure-as-code for the AWS deploy. See
                        terraform/README.md. State backend: S3 bucket
                        scottclarkio-terraform-state (private, versioned).
                        Run terraform commands with the AWS credential
                        bridge: `eval "$(aws configure export-credentials
                        --format env)"` (see Phase 1D bullet for why).
.github/workflows/      GH Actions: deploy.yml fires on push to main.
...
```bash
eval "$(aws configure export-credentials --format env)"   # bridge aws login → env
cd terraform/
...
- **Auto:** Push to `main` triggers `.github/workflows/deploy.yml` → npm ci → astro build → two-pass S3 sync (1y immutable for hashed assets, 1h must-revalidate for HTML/XML/JSON) → CloudFront `/*` invalidate. Steady-state push: ~30s wall clock. The workflow assumes a deploy-only IAM role via OIDC — no AWS keys live in the repo.
- **Manual:** `terraform/` for infrastructure changes (see `terraform/README.md`). `npm run deploy` (= `scripts/deploy.sh`) for emergency hand-deploys; normally redundant with the GH Actions path.
- **AWS infra is live as of 2026-05-05** — provisioned by `terraform apply` against the `terraform/` module. Account `144229378081`. Region `us-east-1`. Re-apply requires the AWS credential bridge (`eval "$(aws configure export-credentials --format env)"`) before `terraform plan`/`apply` — see Phase 1D bullet for why.
- **Two latent bugs caught during the first apply** (both fixed in main): `terraform/variables.tf` had a `${var.domain}` interpolation in its own description (Terraform forbids any interpolation in a `description` field — blocked `terraform init`); `.github/workflows/deploy.yml`'s second `aws s3 sync` had `--exclude "*"` *after* the `--include` flags (last-pattern-wins ordering uploaded zero files, the apex 403'd). Both are documented here so the same patterns don't get re-introduced.
